...
export default class Product {
    ...
    updateCartContent(modal, cartItemHash, onComplete) {
        this.getCartContent(cartItemHash, (err, response) => {
            ...
            const suggestiveCartCarousel = modal.$content.find('.suggestiveCart .productGrid');
            suggestiveCartCarousel.owlCarousel(suggestiveCartCarousel.data('owl'));

            modal.$content.find('.suggestiveCart .productGrid .prod-item').each(function(){
                var product = $(this).find('h4.prod-name a');
                var proId = $(this).find('h4.prod-name a').attr('data-product-id');
